...
public class StreamKBFR extends JPanel implements NativeKeyListener{
  //HashMap to contain all buttons
    protected HashMap<Integer, KBButtonFR> KB = new HashMap<Integer, KBButtonFR>();
	...
	public StreamKBFR(final JFrame frame) {
        ...
        Integer[] numKeyList = {222, 49, 50, 51, 52, 53, 54, 55, 56, 57, 48, 219, 187, 8};
        Integer[] tabLineList = {9, 65, 90, 69, 82, 84, 89, 85, 73, 79, 80, 221, 186, 13};
        Integer[] capsLineList = {20, 81, 83, 68, 70, 71, 72, 74, 75, 76, 77, 192, 220, 1013};
        Integer[] shiftLineList = {160, 226, 87, 88, 67, 86, 66, 78, 188, 190, 191, 223, 161};
        ...
        //manually adding arrow keys for proper formatting
		KB.put((Integer)37, new KBButtonFR(37));
		KB.put((Integer)38, new KBButtonFR(38));
		KB.put((Integer)39, new KBButtonFR(39));
		KB.put((Integer)40, new KBButtonFR(40));
    ...
    private static void makeStreamKB() {
        ...
        JFrame frame = new JFrame("StreamKB 1.1 FR");
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

        //Add content to the window.
        frame.add(new StreamKBFR(frame));
            ...
            SwingUtilities.invokeLater(new Runnable() {
                public void run() {
                	if(KB.get(KBe1.getRawCode()) != null){
                		KB.get(KBe1.getRawCode()).setBackground(hiC);
                	}
                	//Hilight 2nd part of enter key if pressed
                	if(KBe1.getRawCode() == 13){
                		KB.get((Integer)1013).setBackground(hiC);
                	}
                    repaint();
                }
	        });
            ...
            SwingUtilities.invokeLater(new Runnable() {
                public void run() {
                	if(KB.get(KBe2.getRawCode()) != null){
	                	KB.get(KBe2.getRawCode()).setBackground(btC);
	                }
                	//Reset 2nd part of enter key if pressed
                	if(KBe2.getRawCode() == 13){
                		KB.get((Integer)1013).setBackground(btC);
                	}
	                repaint();
                }
	        });
	...
	protected void makeButtons(Integer[] keyList){
		for(Integer I : keyList){
			KB.put(I, new KBButtonFR(I.intValue()));
		}
	}
